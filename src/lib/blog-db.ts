...
export interface BlogPost {
  ...
  categorySlug?: string;
  tags?: string[];
  views?: number; // Added for sorting
  engagement?: number; // Added for sorting (e.g., likes + comments)
}
...
export async function createBlogPost(postData: Omit<BlogPost, '_id' | 'publishDate'> & { publishDateString?: string }): Promise<BlogPost | null> {
  ...
  const newPost: BlogPost = {
    ...postData,
    id: postData.id || postData.title.toLowerCase().replace(/\s+/g, '-').slice(0, 50) + Date.now().toString().slice(-5), // Auto-generate slug if not provided
    publishDate: postData.publishDateString ? new Date(postData.publishDateString) : new Date(),
    views: 0,
    engagement: 0,
    // _id would be generated by a real DB
  };
  MOCK_BLOG_POSTS.unshift(newPost); // Add to the beginning of the array
  console.log("Simulated creating blog post:", newPost);
  return newPost;
}
...
export async function updateBlogPost(postId: string, postData: Partial<Omit<BlogPost, '_id' | 'id' | 'publishDate'>> & { publishDateString?: string }): Promise<BlogPost | null> {
  ...
  const updatedPost = {
    ...MOCK_BLOG_POSTS[postIndex],
    ...postData,
    publishDate: postData.publishDateString ? new Date(postData.publishDateString) : MOCK_BLOG_POSTS[postIndex].publishDate,
  };
  MOCK_BLOG_POSTS[postIndex] = updatedPost;
  console.log("Simulated updating blog post:", updatedPost);
  return updatedPost;
}
...
export async function deleteBlogPost(postId: string): Promise<boolean> {
  ...
  if (indexToRemove > -1) {
    ...
    console.log("Simulated deleting blog post:", postId);
