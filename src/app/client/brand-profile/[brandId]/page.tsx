...
function EditBrandProfilePageContent() {
  const { toast } = useToast();
  const router = useRouter();
  const params = useParams();
  const brandId = params.brandId as string;

  const [formData, setFormData] = useState<BrandProfileFormData>(defaultBrandProfile);
  ...
  useEffect(() => {
    async function loadData() {
      if (!isNew) {
        const kit = await getBrandKitById(brandId);
        if (kit) {
          setFormData(kit);
        } else {
          toast({ title: "Not Found", description: "Brand profile not found.", variant: "destructive"});
          router.push('/client/brand-profiles');
        }
      }
      setIsLoading(false);
    }
    loadData();
  }, [brandId, isNew, router, toast]);


  useDebouncedEffect(
    () => {
      // Auto-save only for existing profiles, not while creating a new one
      if (!isNew && formData.id !== 'default' && (formData.companyName || formData.industry)) {
        setIsSaving(true);
        setIsSaved(false);
        saveBrandKits(prevKits => {
          const index = prevKits.findIndex(k => k.id === formData.id);
          if (index !== -1) {
            const newKits = [...prevKits];
            newKits[index] = formData;
            return newKits;
          }
          return [...prevKits, formData];
        }).then(() => {
          setIsSaving(false);
          setIsSaved(true);
        });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Assign a new ID if it's a new profile
    const submissionData = isNew ? { ...formData, id: `brand_${Date.now()}` } : formData;

    await saveBrandKits(prevKits => {
      const index = prevKits.findIndex(k => k.id === submissionData.id);
      if (index !== -1) {
        const newKits = [...prevKits];
        newKits[index] = submissionData;
        return newKits;
      }
      return [...prevKits, submissionData];
    });

    toast({ title: `Brand Profile ${isNew ? 'Created' : 'Saved'}`, description: `Your brand profile for "${submissionData.companyName}" has been saved.` });
    setIsSaving(false);
    setIsSaved(true);
    router.push('/client/brand-profiles');
  };
  ...
  if (isLoading) {
    return <div className="text-center py-10"><Loader2 className="h-8 w-8 mx-auto animate-spin text-primary" /></div>;
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
            <Button variant="outline" size="icon" asChild>
                <Link href="/client/brand-profiles"><ArrowLeft className="h-4 w-4" /></Link>
            </Button>
            <h1 className="text-3xl font-bold font-headline flex items-center">
              <Sparkles className="mr-3 h-8 w-8 text-primary" />
              {isNew ? 'New Brand Profile' : 'Edit Brand Profile'}
            </h1>
        </div>
        <div className="flex items-center gap-2 text-sm">
            ...
            {isSaved && !isNew && <><Check className="h-4 w-4 text-green-500" /> <span className="text-muted-foreground">Saved</span></>}
            <Button onClick={handleSubmit} disabled={isSaving}>
                <Save className="mr-2 h-4 w-4" /> {isNew ? 'Create Brand' : 'Save Changes'}
            </Button>
        </div>
      </div>
...
export default function EditBrandProfilePage() {
    return (
        <Suspense fallback={<div className="text-center py-10"><Loader2 className="h-8 w-8 mx-auto animate-spin text-primary" /></div>}>
            <EditBrandProfilePageContent />
        </Suspense>
    )
}
