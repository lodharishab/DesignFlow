...
export function PortfolioItemCard({ item, className }: PortfolioItemCardProps) {
  return (
    <Card className={cn("flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 group overflow-hidden", className)}>
      <Link href={`/portfolio/${item.id}`} className="block relative aspect-[4/3] w-full overflow-hidden rounded-t-lg">
          <Image 
      ...
      <CardContent className="p-4 flex-grow flex flex-col">
        ...
        <Link href={`/portfolio?category=${item.categorySlug}`} className="inline-block">
        ...
        <div className="mt-auto flex items-center justify-between pt-2 border-t border-border/50">
          {item.designer && (
            <Link href={`/designers/${item.designer.slug}`} className="flex items-center space-x-1.5 text-xs text-muted-foreground hover:text-primary group/designer">
