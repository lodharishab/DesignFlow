...
import { useState, type ReactElement, useMemo, Fragment } from 'react';
...
// This data would typically come from a store or API.
...
export function OrdersTableView({ fixedStatusFilter }: OrdersTableViewProps): ReactElement {
  ...
  const displayedOrders = useMemo(() => {
    return allOrders.filter(order =>
      fixedStatusFilter === 'All' || order.status === fixedStatusFilter
    ).sort((a, b) => b.orderDate.getTime() - a.orderDate.getTime());
  }, [allOrders, fixedStatusFilter]);
